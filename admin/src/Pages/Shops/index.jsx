...
const Index = () => {
    ...
    const GetTableData = () => {
        API.ProductShop.list({ pag: 1, only_child: "only_child" }).then(res => {
            const { data } = res.data
    ...
    const SearchMarkets = (e, val) => {
        if (val === 'shop') {
            API.ProductShop.list({ pag: 1, name: e.target.value, only_child: "only_child" }).then(res => {
                const { data } = res.data
                setShowTableData([...data?.[1]])
            })
        } else {
            API.ProductShop.list({ pag: 1, code: e.target.value, only_child: "only_child" }).then(res => {
                const { data } = res.data
                setShowTableData([...data?.[1]])
            })
        }
