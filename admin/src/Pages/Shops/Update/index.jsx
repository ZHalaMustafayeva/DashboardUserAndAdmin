...
const Index = ({ onClose, open, editData }) => {
  ...
  const onFinish = (value) => {
    ...
    value.shop_cars = value.shop_cars.filter(val => val);
    setIsLoad(true);
  ...
  const DeleteItem = (id, order_id) => {
    API.Order.delete({ id }).then(res => {
      if (res.status === 200) GetItemData(order_id)
    })
  }
    ...
    >
      ...
      >
        <Row gutter={16} justify='end'>
          <Col span={8} >
            <Form.Item label='Əsas Mağaza:' name='parent_id' className='c-form-item' >
