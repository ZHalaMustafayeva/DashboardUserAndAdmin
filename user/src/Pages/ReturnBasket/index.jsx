...
const Index = () => {
    ...
    const GetAllBasketData = () => {
        API.Order.list({ inserted_user_id: JSON.parse(localStorage.getItem('access_token'))?.id, pag: 1, status: 1, type: 2 }).then(res => {
            const { data } = res.data;
            setStore([...data?.[1]])
        })
    }
    ...
    const showConfirm = () => {
        confirm({
            title: 'Geri qaytarılma yeniləndi.',
            icon: <CheckCircleTwoTone />,
            content: "Digər Geri qaytarılmalarda dəyişiklik etmək istəyirsiniz?",
            onOk() {
                GetAllBasketData()
            },
            okText: "Bəli",
        ...
        <>
            <Row style={{ marginBottom: 16 }}>
                <Space>
                    ...
                    <Link to='/return/markets'>Geri dön</Link>/
                    <Text>Səbət</Text >
                </Space>
            </Row>
            <Row>
            ...
            <Row>
                <Col span={24}>
                    {store?.length ?
                        <Title level={3}>Geri qaytarılanların siyahısı</Title>
                        :
                        <Title level={3}>Gözləmədə olan geri qatarılmanız yoxdur</Title>
                    }
                </Col>
            </Row>
